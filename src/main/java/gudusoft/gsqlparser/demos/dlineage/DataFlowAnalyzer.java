...
import gudusoft.gsqlparser.dlineage.dataflow.model.DataflowRemoveOption;
...
import gudusoft.gsqlparser.dlineage.util.DataflowRemoveHelper;
...
public class DataFlowAnalyzer {
	...
	public static void main(String[] args) {
		if (args.length < 1) {
			...
			System.out.println("/n: Optional, normalize output.");
			...
			System.out.println("/simpleShowRelationTypes: Optional, simple output with specified relation types, support fdd, fdr.");
			System.out.println("/withTemporaryTable: Optional, determine whether to output the temporary tables in simple output, default is false.");
			System.out.println(
			...
			System.out.println("/showResultSetTypes: Optional. This option is valid only when /s or /i option is used, and is used to specify the result set types to be output, separate with commas, result set types contains array, "
					+" struct, result_of, cte, insert_select, update_select, merge_update, merge_insert, output, update_set,\r\n"
					+ "	pivot_table, unpivot_table, alias, rs, function, case_when");
			System.out.println("/showVariable: Optional. This option is valid only when /s or /i option is used, and is used to reserve all the variables and cursors.");
			System.out.println("/showCursor: Optional. This option is valid only when /s or /i option is used, and is used to reserve all the cursors.");
			System.out.println("/showSynonym: Optional. This option is valid only when /s or /i option is used, and is used to reserve all the synonyms.");
			System.out.println("/removeResultSetTypes: Optional. This option is used to remove the specified result set types to be output, separate with commas, result set types contains array, "
					+" struct, result_of, cte, insert_select, update_select, merge_update, merge_insert, output, update_set,\r\n"
					+ "	pivot_table, unpivot_table, alias, rs, function, case_when");
			System.out.println("/removeVariable: Optional. This option is remove all the variables and cursors.");
			System.out.println("/removeCursor: Optional. This option is remove all the cursors.");
			System.out.println("/removeUnusedSynonym: Optional. This option is remove all the unused synonym.");
			System.out.println(
			...
			System.out.println("/graph: Optional, print the json format output with graph information.");
			System.out.println("/stat: Optional, output the analysis statistic information.");
			System.out.println("/tableLineage [/csv /delimiter]: Optional, output table level lineage.");
			System.out.println("/csv: Optional, output column level lineage in csv format.");
			System.out.println("/csv-simple: Optional, output column level lineage in a simplified csv format (source schema.table.column, target schema.table.column, relation type), excluding records that reference the synthetic RelationRows column.");
			...
			System.out.println("/traceTablePosition: Optional, trace all table positions. Default is false.");
			return;
		}
		...
		boolean withGraph = jsonFormat && argList.indexOf("/graph") != -1;
		...
		boolean csvSimple = argList.indexOf("/csv-simple") != -1;
		boolean csv = argList.indexOf("/csv") != -1 || csvSimple;
		boolean lv = argList.indexOf("/lv") != -1;

		String delimiter = (argList.indexOf("/delimiter") != -1 && argList.size() > argList.indexOf("/delimiter") + 1) ? argList.get(argList.indexOf("/delimiter") + 1) : (csvSimple ? ", " : ",");
		if (tableLineage) {
			simple = false;
		...
		if (!stat) {
			...
			dlineage.getOption().setShowERDiagram(argList.indexOf("/showER")!=-1);
			dlineage.setShowCountTableColumn(argList.indexOf("/treatArgumentsInCountFunctionAsDirectDataflow")!=-1);

			dlineage.getOption().setSimpleShowVariable(argList.indexOf("/showVariable")!=-1);
			dlineage.getOption().setSimpleShowCursor(argList.indexOf("/showCursor")!=-1);
			dlineage.getOption().setSimpleShowSynonym(argList.indexOf("/showSynonym")!=-1);
			dlineage.getOption().setIgnoreUnusedSynonym(argList.indexOf("/removeUnusedSynonym")!=-1);
			dlineage.getOption().setShowCaseWhenAsDirect(argList.indexOf("/showCaseWhenAsIndirect")==-1);
			dlineage.getOption().setNormalizeOutput(argList.indexOf("/n") != -1);
			dlineage.getOption().setTraceTablePosition(argList.indexOf("/traceTablePosition")!=-1);

			if (argList.indexOf("/defaultDatabase") != -1 && argList.size() > argList.indexOf("/defaultDatabase") + 1) {
				dlineage.getOption().setDefaultDatabase(args[argList.indexOf("/defaultDatabase") + 1]);
			}
			if (argList.indexOf("/defaultSchema") != -1 && argList.size() > argList.indexOf("/defaultSchema") + 1) {
				dlineage.getOption().setDefaultSchema(args[argList.indexOf("/defaultSchema") + 1]);
			}

			if(argList.indexOf("/showResultSetTypes")!=-1 && argList.size() > argList.indexOf("/showResultSetTypes") + 1) {
			...
			if (argList.indexOf("/filterRelationTypes") != -1 && argList.size() > argList.indexOf("/filterRelationTypes") + 1) {
				String filterRelationTypes = args[argList.indexOf("/filterRelationTypes") + 1];
				if (filterRelationTypes != null) {
					dlineage.getOption().filterRelationTypes(filterRelationTypes.split(","));
				}
			}

			if (argList.indexOf("/s") != -1 && argList.indexOf("/simpleShowRelationTypes") != -1 && argList.size() > argList.indexOf("/simpleShowRelationTypes") + 1) {
				String simpleShowRelationTypes = args[argList.indexOf("/simpleShowRelationTypes") + 1];
				if (simpleShowRelationTypes != null) {
					dlineage.getOption().setSimpleShowRelationTypes(simpleShowRelationTypes.split(","));
				}
			}
			...
			DataflowRemoveOption removeOption = null;
			if (argList.indexOf("/removeVariable") != -1 || argList.indexOf("/removeCursor") != -1 || argList.indexOf("/removeResultSetTypes") != -1 || ignoreFunction) {
				removeOption = new DataflowRemoveOption();
				removeOption.setRemoveVariable(argList.indexOf("/removeVariable") != -1);
				removeOption.setRemoveCursor(argList.indexOf("/removeCursor") != -1);

				if (argList.indexOf("/removeResultSetTypes") != -1 && argList.size() > argList.indexOf("/removeResultSetTypes") + 1) {
					String resultSetTypes = args[argList.indexOf("/removeResultSetTypes") + 1];
					if (resultSetTypes != null) {
						removeOption.setRemoveResultSetTypes(resultSetTypes.split(","));
					}
				}

				if (ignoreFunction) {
					removeOption.setRemoveResultSetTypes(ResultSetType.function);
				}
			}

//			dlineage.setShowConstantTable(true);
//			dlineage.getOption().setTraceProcedure(true);
			...
			if (tableLineage) {
				...
				if(removeOption!=null){
					originDataflow = new DataflowRemoveHelper().remove(originDataflow, vendor, removeOption);
				}
				if (csv) {
					...
					if (lv) {
						try {
							result = new DataFlowGraphGenerator().genDlineageGraph(vendor, true, dataflow);
						} catch (Exception e) {
							e.printStackTrace();
							result = null;
						}
						...
						if(withGraph){
							try {
								if(argList.indexOf("/showER")!=-1){
									result = new DataFlowGraphGenerator().genERGraph(vendor, dataflow);
								}
								else {
									result = new DataFlowGraphGenerator().genDataflowGraph(model);
								}
							} catch (Exception e) {
								e.printStackTrace();
								result = null;
				...
				if(removeOption!=null){
					dataflow = new DataflowRemoveHelper().remove(dataflow, vendor, removeOption);
				}
				if (lv) {
					try {
						result = new DataFlowGraphGenerator().genDlineageGraph(vendor, true, dataflow);
					...
					result = ProcessUtility.generateColumnLevelLineageCsvSimple(dlineage, dataflow, delimiter);
				}
				else if (csv) {
				...
				try {
					...
					DataFlowHandleAdapter listener = new DataFlowHandleAdapter() {
						...
						public void endAnalyzeDataFlow(TGSqlParser sqlparser) {
							...
							totalLines[0] += (endToken.lineNo + endToken.getAstext().split("\n").length - 1);
						}
