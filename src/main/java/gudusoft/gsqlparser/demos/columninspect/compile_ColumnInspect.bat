...
call setenv\setenv.bat
    ...
    echo ***************************
    echo.
    cd src\main\java\gudusoft\gsqlparser\demos\columninspect
    pause
    goto END
    )
...
%JAVAC_CMD% -d %targetdir% -encoding UTF-8 -sourcepath src\main\java -classpath %CLASSPATH% src\main\java\gudusoft\gsqlparser\demos\columninspect\ColumnInspect.java
