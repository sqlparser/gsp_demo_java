...
import gudusoft.gsqlparser.stmt.mssql.TMssqlTryCatch;
...
public class Analyze_SP
{
			...
			TCustomSqlStatement stmt )
	{
		...
		else if ( stmt instanceof TMssqlTryCatch )
		{
			TMssqlTryCatch tryCatch = (TMssqlTryCatch) stmt;
			if ( tryCatch.getTryStatements( ) != null )
			{
				for ( int i = 0; i < tryCatch.getTryStatements( ).size( ); i++ )
				{
					analyzeSqlStatement( procedureInfo,
							tryCatch.getTryStatements( ).get( i ) );
				}
			}
			if ( tryCatch.getCatchStatements( ) != null )
			{
				for ( int i = 0; i < tryCatch.getCatchStatements( ).size( ); i++ )
				{
					analyzeSqlStatement( procedureInfo,
							tryCatch.getCatchStatements( ).get( i ) );
				}
			}
		}
			...
			if ( token.getDbObjType( ) == TObjectName.ttobjFunctionName )
			{
				...
				for ( int j = 0; j < list.size( ); j++ )
				{
					...
					if ( node instanceof TFunctionCall )
					{
						...
						function.function = token.getAstext();
			...
			TMssqlCreateProcedure procedure )
	{
		...
		for ( int i = 0; i < tokenList.size( ); i++ )
		{
			...
			if ( token.tokentype == ETokenType.ttkeyword
					&& token.getAstext().trim( ).equalsIgnoreCase( "try" ) )
			{
