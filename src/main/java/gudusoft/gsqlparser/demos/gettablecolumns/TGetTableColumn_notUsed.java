...
public class TGetTableColumn_notUsed {
            ...
            for(int i=0;i< sourceTokenList.size();i++) {
                ...
                if (startIndex != -1 && endIndex != -1) {
                    ...
                    for (int j = startIndex + 1; j <= endIndex; j++) {
                        ...
                        firstToken.setString(firstToken.getAstext() + st.getAstext());
                    }
    ...
    String dotChar = ".";
    public TGetTableColumn_notUsed(EDbVendor pDBVendor){
       dbVendor = pDBVendor;
    ...
    protected void analyzeStmt(TCustomSqlStatement stmt, int pNest){
        ...
        for(int i=0;i<stmt.tables.size();i++){
            ...
            //{
             ...
             if (showColumnsOfCTE && lcTable.isCTEName()){
                 for(TAttributeNode node:lcTable.getAttributes()){
                     if ((node.getName().endsWith("*")) && (!showStarColumnOfCTE)) continue;
            ...
            else if (lcTable.getTableType() == ETableSource.lateralView){
                tn = "(lateral_view:"
                        + lcTable.getTableName( )
                        + ")";
            }
...
//                }
        ...
        if (stmt.getOrphanColumns().size() > 0){
            ...
            for (int k=0;k<stmt.getOrphanColumns().size();k++){
                ...
                TObjectName column = stmt.getOrphanColumns().getObjectName(k);
//                if (column.isOrphanColumn()){
//                    System.out.println("Candidate table size of column: "+ column.toString()+" ,"+ column.getCandidateTables().size());
//                }
