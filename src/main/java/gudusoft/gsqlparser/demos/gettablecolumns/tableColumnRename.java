...
public class tableColumnRename
{
	...
	public String getModifiedText( )
	{
		...
		{
			sb.append(sqlparser.sourcetokenlist.get(i).getAstext());
		}
		...
		for ( int k = 0; k < stmt.tables.size( ); k++ )
		{
			...
			for ( int m = 0; m < table.getLinkedColumns( ).size( ); m++ )
			{
				...
				if ( isfound )
				{
					...
					{
						st.setString( this.targetTable + "." + st.getAstext());
					}
					...
					if ( this.targetSchema != null )
					{
						...
						{
							st.setString( this.targetSchema + "." + st.getAstext());
						}
					}
					else
					{
		...
		for ( int k = 0; k < stmt.tables.size( ); k++ )
		{
			...
			for ( int m = 0; m < table.getLinkedColumns( ).size( ); m++ )
			{
								...
								.equalsIgnoreCase( this.sourceSchema ) )
				{
					...
					if (column.getSchemaToken().getNextTokenInChain().getAstext().equals(".")){
							...
							.equalsIgnoreCase( this.sourceSchema ) )
			{
				...
				if (table.getTableName().getSchemaToken().getNextTokenInChain().getAstext().equals(".")){
