package gudusoft.gsqlparser.dlineage.dataflow.metadata.sqldep;
...
import gudusoft.gsqlparser.dlineage.dataflow.metadata.MetadataAnalyzer;
...
public class SQLDepMetadataAnalyzer implements MetadataAnalyzer {
	...
	private Map<String, procedure> procedureMap = new LinkedHashMap<String, procedure>();
		...
		if (dataflow.getTables() != null) {
			for (table table : dataflow.getTables()) {
				Collections.sort(table.getColumns(), new Comparator<column>() {
					public int compare(column t1, column t2) {
