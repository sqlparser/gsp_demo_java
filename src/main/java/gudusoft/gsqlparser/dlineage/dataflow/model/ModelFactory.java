...
//        }
        ...
        if("*".equals(column.getColumnNameOnly()) && column.getSourceTable()!=null){
        	...
        	Map<String, TObjectName> columnMap = new LinkedHashMap<String, TObjectName>();
			for (int i = 0; i < columns.size(); i++) {
			...
			for (int i = 0; i < columns.size(); i++) {
				...
				String columnName = item.getColumnNameOnly();
				if (!item.getSourceTable().equals(column.getSourceTable()) && !"*".equals(columnName)) {
					String key = SQLUtil.getIdentifierNormalName(columnName);
					if (!columnMap.containsKey(key)) {
						columnMap.put(key, item);
					}
				}
			}
        	columnModel.bindStarLinkColumns(new LinkedHashMap<String, Set<TObjectName>>());
        }
