package demos.dlineage.sqlenv.grabit;
...
public class MultipleGrabitSQLEnv extends TSQLEnv {
	...
	public MultipleGrabitSQLEnv(EDbVendor dbVendor, JSONArray jsonArray) {
		super(dbVendor);
		...
		synchronized (init) {
			...
			for (int x = 0; x < jsonArray.size(); x++) {
				...
				try {
					...
					Logger.getLogger(MultipleGrabitSQLEnv.class.getName()).log(Level.WARNING, "Parse json failed.", e);
				}
			}
