...
public class GreenplumConnector implements Connector {
	...
	public Connection toConnect(TSQLDataSource datasource)
		...
		try {
			...
			return ConnectUtil.getConnection(datasource.getAccount(), datasource.getPassword(), ConnectConstant.Greenplum.getDriver(), String
					.format(ConnectConstant.Greenplum.getUrl(), address, datasource.getDatabase()).replace(";DatabaseName=null", ""),
					 datasource.getTimeout());
		}
	}
