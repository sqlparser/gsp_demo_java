 ...
 * CemB
 */
public class PostgreSQLJSONExport extends AbstractJSONExport {
    ...
    protected String tablesSql(TSQLDataSource datasource, String database, String schema) {
        return SQLFileUtil.readSqlContent(datasource.getDbCategory(), "schema.sql").replace("%s", new Identifier(EDbVendor.dbvpostgresql, ESQLDataObjectType.dotCatalog, database)
				.getNormalizeIdentifier());
    }

    @Override
    protected String queriesSql(TSQLDataSource datasource, String database, String schema) {
        return SQLFileUtil.readSqlContent(datasource.getDbCategory(), "query.sql").replace("%s", new Identifier(EDbVendor.dbvpostgresql, ESQLDataObjectType.dotCatalog, database)
				.getNormalizeIdentifier());
    }
